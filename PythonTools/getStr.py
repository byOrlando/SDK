...
name = ["大贤", "徐子苓", "倪鸿", "徐桢立", "唐顺宗李诵", "谢旌", "励守谦", "锅岛直茂", "陈谟", "柏景伟", "张卤", "童钰", "周宗建", "桑戴克", "田穰苴", "阎宗临",
        "博尔忽", "曹锡璜", "秦炳文", "汉质帝", "忽必来", "商思源", "许昭", "张行简", "百里溪", "欧阳章", "黄子君", "河间国", "虞泳", "靳玉砚", "周茂兰",
        "王廷魁", "朱汝华", "保田圭", "冯一萌", "陆廷灿", "酒井忠胜", "林彦峰", "丸山隆平", "杨应奎", "米拉蒙特斯", "蒋国榜", "雷恪", "胡助", "周星诒", "梅宫安娜",
        "山本百合子", "山崎和佳奈", "杵臼", "郝莲", "黄祯", "黄葆戊", "柴宗训", "魏校", "管念慈", "石颐", "王寂", "劳之辨", "莫德格玛", "克里斯托弗哥伦布", "青海骢",
        "王宏撰", "朱彭年", "朱伦瀚", "张纯修", "寄尘", "梁宰豪", "顾汝修", "项维仁", "张龙德", "李睍", "金蓉镜", "赵南哲", "小岛南", "清世祖", "袁廷梼", "何镗",
        "田崎敬浩", "袁茂峰", "姬封", "速不台", "吴观岱", "齐召南", "周健君", "樱井莉亚", "伍骥", "荣霖", "张锡兰", "窦光鼐", "钱陆灿", "方孝标", "沈瀚", "唐复",
        "王鎏", "汤世澍", "山口胜平", "赵备", "梁玉绳", "殷兆镛", "保巴", "胡俨", "杨应春", "应仁", "濑户朝香", "汪端", "姚洪业", "阎兴邦", "庄臻凤",
        "吕得", "杜思敬", "陈仁锡", "风清扬", "章懋", "宋汶", "瑜妃", "祖册", "吴宝秀", "滕弘", "长孙晟", "冯文蔚", "苑田勇一", "绿川光", "戈涛", "舜天齐", "蒋溥",
        "胡一鹏", "陈璜", "宣永光", "黄鸣龙", "瑞迪克", "夏侯惠", "小早川隆景", "胡煦", "顾鼎臣", "杜文澜", "陈博生", "黄绍统", "宋昭公", "康绍镛", "廪人", "高焕文",
        "丰臣秀赖", "稻森泉", "华核", "加藤罗莎", "沈岸登", "中丸雄一", "鲁贞", "杨赓笙", "巴多里奥", "戴之奇", "缪昌期", "张天英", "费观", "殷奎", "大内义隆", "唐大烈",
        "刁包", "诸葛尚", "坂本昌行", "布劳岱尔", "朱友珪", "何旃理", "徐霈", "游本嘉", "关松房", "郭利君", "栗原惠", "汪喜孙", "张汝霖", "马雄镇", "靳祥", "卢周来",
        "渡哲也", "陈际泰", "嵇曾筠", "陶宏景", "姚明恭", "立花慎之介", "章穆", "龟井兹矩", "松本零士", "王友石", "周体仁", "董讷", "邵谊", "钟复", "冈本圭人", "慧南",
        "吴一舟", "胡世宁", "李敬思", "钱廉", "雷以诚", "刘寔", "赵浚", "张羽材", "世阿弥", "康建刚", "张大受", "崔晓光", "小池荣子", "邓传安", "汪洪度", "伍代夏子",
        "嘉数由美", "艾县", "郝浴", "胡方平", "辛宪英", "王贞仪", "生天目仁美", "鲍刚", "程颂万", "陆师道", "鲁炀公", "荀正", "堤真一", "千叶真一", "齐文公", "赵贞吉"]
...
xx = name[random.randint(0, 220)]
quotes = data["famous"]  # a 代表前面垫话，b代表后面垫话
frontDianhua = data["before"]  # 在名人名言前面弄点废话
behindDianHua = data['after']  # 在名人名言后面弄点废话
废话 = data['bosh']  # 代表文章主要废话来源

duplication = 2


def shufflingThrough(datalist):
    global duplication
    pools = list(datalist) * duplication
    while True:
...
after_bosh = shufflingThrough(废话)
after_famous = shufflingThrough(quotes)


def create_famous():
    global after_famous
    xx = next(after_famous)
    xx = xx.replace("a", random.choice(frontDianhua))
    xx = xx.replace("b", random.choice(behindDianHua))
    return xx
